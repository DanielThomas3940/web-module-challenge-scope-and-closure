...
function scoreboard(inningScore, inning, inningNumber) {
  const scoreByInning = [];
  let homeScore = 0;
  let awayScore = 0;

  for(let i = 0; i < inningNumber; i++){
    const thisInning = inningScore(inning);
    homeScore = homeScore + thisInning.Home
    awayScore = awayScore + thisInning.Away
    scoreByInning.push( `Inning ${ i =1}: Away ${thisInning.away} - Home ${thisInning.Home}`)
  }
  if(homeScore = awayScore){
    scoreByInning.push(`This game will require extra innings: Away ${thisInning.Away} - Home ${thisInning.Home} ${thisInning.Home}`)
  }else{
    scoreByInning.push(`Final Score: Away: ${awayScore} - Homee ${homeScore}`);
  }
